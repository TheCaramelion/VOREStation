...
/datum/component/pinata/Initialize(
	minimum_damage = 10,
	drop_chance = 40,
	candy = list(/obj/item/reagent_containers/food/snacks/candy, /obj/item/clothing/mask/chewable/candy/lolli, /obj/item/reagent_containers/food/snacks/chocolatebar, /obj/item/clothing/mask/chewable/candy/gum),
	death_drop = 5
)

	src.minimum_damage = minumum_damage
	...
	if(ismob(parent))
		...
		RegisterSignal(parent, COMSIG_MOB_DEATH, PROC_REF(pinata_broken))
		return
	if(isobj(parent))
...
/datum/component/pinata/proc/damage_inflicted(obj/target, damage, damage_type, ...)
	SIGNAL_HANDLER
	if(damage < minimum_damage || damage_type == HALLOSS || damage_type == OXY)
		return
	if(!prob(drop_chance + damage))
		return
	var/list/turf_options = get_adjacent_turfs(parent)
