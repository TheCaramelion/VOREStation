...
/datum/component/remote_materials
	...
	var/obj/machinery/ore_silo/silo = null
...
/datum/component/remote_materials/Initialize(category, mapload, allow_standalone = TRUE, force_connect = FALSE, mat_container_flags = NONE)
	...
	if(force_connect || (mapload && isStationLevel(T.z)))
...
/datum/component/remote_materials/proc/is_valid_link(atom/targeta, atom/targetb = silo)
	return ((isStationLevel(targeta.z) && isStationLevel(targetb.z)))
...
/datum/component/remote_materials/proc/receive_buffer(datum/source, mob/user, datum/buffer, obj/item/buffer_parent)
	if(!QDELETED(buffer) && istype(buffer, /obj/machinery/ore_silo))
		...
		if(!is_valid_link(P, buffer))
			to_chat(user, span_warning(span_warning("[parent]'s material manager blinks red: Out of range.")))
			return
		if(silo == buffer)
...
/datum/component/remote_materials/proc/silo_log(var/obj/machinery/M, action, amount, noun, list/mats)
	if(silo)
...
/datum/component/remote_materials/proc/eject_sheets(datum/material/material_ref, eject_amount)
	...
	if(!mat_container)
		...
		return FALSE
	if(on_hold())
		movable_parent.atom_say("Mineral access is on hold, please contract the quartermaster.")
		return FALSE
