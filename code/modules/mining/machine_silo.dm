...
/obj/machinery/ore_silo
	name = "ore_silo"
	desc = "An all-in-one bluespace storage and trasmission system for the station's mineral distribution needs."
...
/obj/machinery/ore_silo/Initialize(mapload)
	. = ..()
	var/static/list/material_list = list(
		/datum/material/iron,
	...
	AddComponent(/datum/component/material_container, material_list, INFINITY, MATCONTAINER_NO_INSERT, /obj/item/stack)

	if(!GLOB.ore_silo_default && mapload && isStationLevel(z))
...
/obj/machinery/ore_silo/attackby(obj/item/O, mob/user, params)
	...
	if(istype(O, /obj/item/stack))
		return remote_attackby(src, user, O)

	return ..()

/obj/machinery/ore_silo/Topic(mob/user)
...
/obj/machinery/ore_silo/proc/on_object_saved(var/depth = 0)
	if(depth >= 10)
		return ""
	var/dat
	...
	for(var/each in material_holder.materials)
		...
		while(amount > 0)
			...
			dat += "[dat ? ",\n" : ""][material_datum.sheet_singular_name]{\n\tamount = [amount_in_stack]\n\t}"
	return dat
